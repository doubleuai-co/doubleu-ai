    ...
    <section className="bg-white px-2 py-4 text-[#0B0A0A] sm:px-6 lg:px-8">
      <div className="container mx-auto mb-12 text-center">
        ...
        <p className="mx-auto max-w-2xl text-md lg:text-lg text-gray-600">
          It&apos;s more than automation, it&apos;s smarter work, seamless integration,
          and real results.
        </p>
      </div>

      <div className="container mx-auto grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 gap-x-12 px-4 lg:px-14">
        {features.map((feature, index) => (
          <div
            key={index}
            className={`
              group px-8 py-14 rounded-xl shadow-md border text-center
              ...
              ${
                feature.highlight
                  ...
                  : "border-gray-100 bg-gray-50 text-gray-800" + // Light background for regular cards
                    " hover:bg-[#2a2a4e] animate-blob animation-delay-2000 hover:text-white hover:border-[#6a5acd]" // Hover glow effect
              }
            `}
          >
            {/* Icon with background circle - Centering is handled by flex, items-center, justify-center */}
            <div
              className={`mb-4 flex h-12 w-12 items-center justify-center mx-auto rounded-md transition-colors duration-300 ease-in-out
                ${
                  feature.highlight
                    ? "bg-gray-50" // Icon background color for highlighted
                    : "bg-gradient-to-r from-[#7e73c3b7] to-[#00b4d85f] hover:bg-white group-hover:bg-[#6a5acd]" // Icon background for regular, changes on hover
                }
              `}
            >
              ...
              <Image
                ...
                className={`object-contain flex-shrink-0 transition-all duration-300 ease-in-out
                  ${feature.highlight ? "filter brightness-200" : "filter brightness-50 group-hover:brightness-200"}
                `}
              />
            </div>
            <h3
              className={`mb-2 text-xl font-semibold transition-colors duration-300 ease-in-out
                ${feature.highlight ? "text-white" : "text-[#222831] group-hover:text-white"}`}
            >
              {feature.title}
            </h3>
            <p
              className={`text-base transition-colors duration-300 ease-in-out
                ${feature.highlight ? "text-gray-300" : "text-gray-700 group-hover:text-white"}`}
            >
              {feature.description}
            </p>
          </div>
      ...
      <div className="mt-16 text-center">
        <Link href="/explore-ai-workers" passHref>
          <button className="rounded-md bg-[#6a5acd] px-8 py-4 text-md font-semibold text-white shadow-lg transition duration-300 ease-in-out hover:bg-[#7b68ee]">
            Explore AI Workers
          </button>
        </Link>
      </div>
